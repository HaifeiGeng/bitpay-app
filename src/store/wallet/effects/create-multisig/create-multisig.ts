...
import {createWalletWithOpts, createReadonlyWalletWithOpts, createOrOpenWalletWithOpts} from '../create/create';
import {
...
export const startCreateKeyMultisig =
  ...
  /**
   * 改造，如果钱包存在，则开启钱包，如果钱包不存在，则创建钱包
   * @param opts 
   * @returns 
   */
  export const startCreateOrOpenReadonlyKeyMultisig =
  (opts: Partial<KeyOptions>): Effect =>
  async (dispatch, getState): Promise<Key> => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // const _wallet = await createWalletWithOpts({key: _key, opts});
        const _wallet = await createOrOpenWalletWithOpts({key: _key, opts});

        console.log(`---------- 多签 创建钱包 startCreateOrOpenReadonlyKeyMultisig 已经获取到 _wallet = [${JSON.stringify(_wallet)}]`);
