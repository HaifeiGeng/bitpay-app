 ...
 * @returns
 */
const RecoveryColdWallet = () => {
  const {t} = useTranslation();
  const dispatch = useDispatch();
  const logger = useLogger();
  const navigation = useNavigation();
  const route = useRoute<RouteProp<WalletStackParamList, 'ImportColdWallet'>>();
  ...
  const setKeyOptions = (
    ...
    if (derivationPathEnabled || recreateWallet) {
      ...
      keyOpts.derivationPath = advancedOpts.derivationPath;
  ...
  const importWallet = async (
    importData: {words: string},
    opts: Partial<KeyOptions>,
  ): Promise<void> => {
    try {
      opts.name = dispatch(GetName(advancedOptions.coin!, advancedOptions.chain));
      opts.mnemonic = importData.words;
      opts.seedType = 'cold';
      // 增加冷钱包标记
      opts.cold = '1';
      ...
      dispatch(
        Analytics.track('Imported Cold Wallet', {
          context: route.params?.context || '',
...
export default RecoveryColdWallet;
